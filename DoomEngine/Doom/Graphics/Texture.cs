...
namespace DoomEngine.Doom.Graphics
{
	...
	public sealed class Texture
	{
		...
		public Texture(string name, bool masked, int width, int height, TexturePatch[] patches)
		{
			this.name = name;
			this.masked = masked;
			this.width = width;
			...
			for (var x = 0; x < width; x++)
			{
				...
				if (patchCount[x] >= 2)
				{
					...
					foreach (var patch in patches)
					{
						...
						Texture.DrawColumnInCache(patchColumn, column.Data, column.Offset, patch.OriginY, height);
					}
		...
		private static void DrawColumnInCache(Column[] source, byte[] destination, int destinationOffset, int destinationY, int destinationHeight)
		{
