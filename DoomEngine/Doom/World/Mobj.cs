...
namespace DoomEngine.Doom.World
{
	...
	public class Mobj : Thinker
	{
		...
		{
			// Momentum movement.
			if (this.momX != Fixed.Zero || this.momY != Fixed.Zero || (this.flags & MobjFlags.SkullFly) != 0)
			{
		...
		private int GetTics(MobjStateDef state)
		{
			...
			{
				if ((int) MobjState.SargRun1 <= state.Number && state.Number <= (int) MobjState.SargPain2)
				{
					return state.Tics >> 1;
				}
				else
		...
		private void NightmareRespawn()
		{
			...
			var fog1 = ta.SpawnMobj(this.x, this.y, this.subsector.Sector.FloorHeight, MobjType.Tfog);
			...
			var fog2 = ta.SpawnMobj(sp.X, sp.Y, ss.Sector.FloorHeight, MobjType.Tfog);
