...
namespace DoomEngine.Doom.World
{
	...
	public sealed class ThingMovement
    {
        ...
        private bool CheckThing(Mobj thing)
        {
            ...
            if (Fixed.Abs(thing.X - this.currentX) >= blockDist ||
                Fixed.Abs(thing.Y - this.currentY) >= blockDist)
            {
                // Didn't hit it.
                return true;
            }
        ...
        public bool TryMove(Mobj thing, Fixed x, Fixed y)
        {
            ...
            if ((thing.Flags & MobjFlags.NoClip) == 0)
            {
                ...
                if ((thing.Flags & MobjFlags.Teleport) == 0 &&
                    this.currentCeilingZ - thing.Z < thing.Height)
                {
                ...
                if ((thing.Flags & MobjFlags.Teleport) == 0 &&
                    this.currentFloorZ - thing.Z > Fixed.FromInt(24))
                {
                    // Too big a step up.
                    return false;
                }
                ...
                if ((thing.Flags & (MobjFlags.DropOff | MobjFlags.Float)) == 0 &&
                    this.currentFloorZ - this.currentDropoffZ > Fixed.FromInt(24))
                {
